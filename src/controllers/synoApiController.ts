...
export class SynoApiController {
  ...
  public static async folder(req: Request, res: Response): Promise<void> {
    try {
      ...
      let url =
        `${SynoApiController.baseUrl}/AudioStation/folder.cgi?api=SYNO.AudioStation.Folder&version=3&method=list` +
  ...
  public static async artist(req: Request, res: Response): Promise<void> {
    try {
      ...
      let url =
        `${SynoApiController.baseUrl}/AudioStation/artist.cgi?api=SYNO.AudioStation.Artist&version=4&method=list` +
        `&_sid=${SynoApiController.sid}` +
        `&limit=${limit}` +
        `&offset=${offset}` +
        `&sort_by=${sortBy}` +
  ...
  public static async album(req: Request, res: Response): Promise<void> {
    try {
      ...
      let url =
        `${SynoApiController.baseUrl}/AudioStation/album.cgi?api=SYNO.AudioStation.Album&version=3&method=list` +
        `&_sid=${SynoApiController.sid}` +
        `&limit=${limit}` +
        `&offset=${offset}` +
        `&sort_by=${sortBy}` +
  ...
  public static async song(req: Request, res: Response): Promise<void> {
    try {
      ...
      let url =
        `${SynoApiController.baseUrl}/AudioStation/song.cgi?api=SYNO.AudioStation.Song&version=3&method=list` +
        `&_sid=${SynoApiController.sid}` +
        `&limit=${limit}` +
        `&offset=${offset}` +
        `&sort_by=${sortBy}` +
